...
- PWA (`web/vite.config.ts`, `vite-plugin-pwa`): `workbox.runtimeCaching` para lecturas offline usa `NetworkFirst` acotado a GET de `/api/v1/**` **excluyendo siempre `/api/v1/auth/**`** — nunca cachear en disco nada que pueda contener tokens/credenciales. TTL corto (5 min) + `networkTimeoutSeconds` bajo para no servir datos viejos si hay red. `navigateFallback` a `/index.html` necesita `/api/` en su denylist, si no las rutas de API caen al shell de React en vez de responder JSON.
- Banners/tours dismissibles del lado cliente (`LocationConsentBanner`, `WelcomeTour`) siguen el mismo patrón: helper propio en `web/src/lib/` con `get*()`/`set*()` sobre una clave dedicada de `localStorage` (nunca una clave genérica compartida), sin backend ni dependencia nueva — el componente lee el flag en el primer render y no vuelve a mostrarse una vez marcado. Reusar este patrón para cualquier UI "mostrar una sola vez" nueva en vez de armar un mecanismo distinto.
