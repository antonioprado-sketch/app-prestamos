...
- **Antes de cada `git commit`+`git push`, sin excepción**: actualizar los tres documentos vivos — `project_state.md` (estado, ya cubierto arriba), `cmem.md` (narrativa portable — agregar sección nueva cuando hubo decisiones de arquitectura/patrón desde su última edición, no solo en cierres de fase) y `CLAUDE.md` (este archivo — actualizar cuando se agregan módulos nuevos, convenciones o patrones reusables que una sesión futura necesita conocer sin tener que re-derivarlos leyendo código). Es la única vía por la que el contexto viaja entre máquinas/sesiones (la memoria de conversación no sincroniza) — dejar alguno desactualizado significa que la próxima sesión arranca con contexto viejo.
