...
**Fase 3 (Administrador) — primer corte hecho**: listar/aprobar/rechazar/pedir corrección de solicitudes (`/admin/solicitudes`, ver arriba). Falta del roadmap de Fase 3: asignar cobradores, `payments` (registrar pagos reales — hoy no existe módulo de pagos), reglas configurables (`score_rules`/`business_rules`), score, BI, ubicaciones. Cada uno debe confirmarse con el usuario como corte aparte antes de implementar, igual que los anteriores.
