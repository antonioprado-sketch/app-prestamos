...
- **UX #1 (fotos solo con c�mara):** se pregunt� al usuario y eligi� la
  opci�n fuerte � "C�mara forzada getUserMedia (Recomendado)" � en vez del
  atributo `capture="environment"` (que es solo una sugerencia al navegador y
  muchos siguen ofreciendo la galer�a). `DocumentsPage` ya no tiene inputs de
  ...
  pidi� el usuario. Lecci�n aprendida a transmitir: en el celular por
  `http://192.168.68.71` la c�mara no se abrir� (contexto seguro obligatorio) �
  se ver� en producci�n HTTPS. El test usa jsdom donde `navigator.mediaDevices`
  no existe, as� que el flujo que se prueba es el del error amigable + cerrar.
- **UX #2 (cobrador, monto precargado + "+/-"):** el dato ya viajaba en la API
  ...
  pr�xima no pagada), los botones -/+ suman/quitan cuotas sin permitir edici�n
  manual del monto, y el total se env�a tal cual. `crypto.randomUUID()` sigue
  como `idempotencyKey`.
- **UX #3 (slider + tope por cliente nuevo):** se necesitaba un endpoint nuevo
  (`GET /api/v1/loans/quote-limit`) para que el slider supiera su tope m�ximo
  seg�n la sesi�n (an�nimo/cliente nuevo ? $3,000; cliente no nuevo ? sin
  tope ? 20,000). La ruta se declar� antes de `@Get(':id')` (orden NestJS, si
  ...
  conserv� porque un test existente lo usa.
...
## Documentación completa del proyecto (2026-08-18)

A pedido del usuario ("documenta todo") se generaron tres documentos nuevos y
se expandió el README. Solo documentación, cero cambios de código:

- `docs/architecture.md` — arquitectura técnica completa (reemplaza el stub):
  componentes/diagrama, stack, estructura del monorepo, patrones backend
  (módulos `@Global()`, funciones puras con TDD, errores/validación,
  seguridad), modelo de datos con enums, flujos de negocio (ciclo de vida del
  préstamo, cotización 1.4×20/10, pagos/penalización, score, reglas
  configurables, aumento de crédito, notificaciones), rutas del frontend por
  rol, carga diferida (Leaflet/MediaPipe/Recharts), PWA, `stitch/`,
  infraestructura/despliegue y decisiones de diseño clave.
- `docs/api-reference.md` — referencia exhaustiva de `/api/v1`: todos los
  endpoints por área (método, ruta, roles, descripción), códigos de estado
  frecuentes y convenciones (Bearer, formato de error, ownership → 404).
- `docs/user-guide.md` — guía de uso por rol: cliente (registro, calculadora,
  onboarding, documentos con cámara, video, pagaré, aumento de crédito),
  cobrador (cartera, cobro precargado con "+/-", aumentos) y administrador
  (BI, solicitudes, clientes, usuarios, configuración, ubicaciones, aumentos).
- `README.md` — sección "Funcionalidades por rol" + enlaces a los tres
  documentos; corregida la nota de `stitch/` (`stitch/cliente/` es el diseño
  de referencia vigente; el resto de mocks ya fue eliminado del repo).

Datos para documentar se recolectaron del código real (schema Prisma,
controllers, router de la web, `business-rules.constants.ts`) — no de memoria.
Pendientes heredados sin cambio: pasada visual en navegador y cámara/geo/push
por HTTPS (Fase 8).
## Sincronizaci�n 2026-08-27 � perfil, reglas visibles, admin reset manual, GPS gate, admin usuarios buscador, Playwright siempre

**Perfil cliente (/app/cliente/perfil) + reglas con palomitas:** password.policy.ts endurecida 8�64 may�s/min�s/n�mero/especial + PasswordRules.tsx checklist en vivo aplicado en Registro, Cambio y Perfil. ClientProfilePage muestra tel�fono no editable, correo, nombres/apellidos, �ltima conexi�n (AuditLog login o updatedAt) y cambio con currentPassword + email de confirmaci�n (y env�o de nueva a correo si existe). Ruta App.tsx /app/cliente/perfil.

**Admin reset manual:** dmin-users.service.ts resetPassword ahora recibe 
ewPassword validado y env�a a User.email ?? Customer.email con mustChangePassword, mail Sent. FE modal manual con PasswordRules. Test dmin-users.e2e-spec.ts actualizado.

**GPS gate en login** (implementado y revertido a pedido): location.ts ensureGpsGranted + LoginPage gate para CLIENT/COLLECTOR con animaci�n ??? y Activar GPS (admin exento). Detect� LoginPage.test.tsx 2 failed por isSecureContext en jsdom � corregido con mock isSecureContext:true + geolocation. Luego revertido a login directo sin GPS ni InstallButton a pedido del usuario.
...
Build 688→412KB, vitest 44 pass, api 72 pass, playwright 1 passed.

## Fix 2026-08-28 — video celular + visor admin (Mita aprobado)

**Síntoma:** `POST /documents` Android 1.9MB `400` y `HEAD /documentos/... 403` en Nginx, preview negro en celular y admin video ya ok tras primer fix pero foto/PDF redirigía a `/` por `window.open` bloqueado.

**Causa 1 — firma MinIO:** `docker-compose.dev.yml:59` hardcodeado `9000` vs `.env:443` → URL `https://:9000` vs proxy `Host` sin puerto → `SignatureDoesNotMatch`. Fix genérico `${MINIO_PUBLIC_PORT}` + `MINIO_PUBLIC_USE_SSL`, URL sin `:9000` → `200`.

**Causa 2 — mime codec:** `video/webm;codecs=vp8,opus` con coma mal-parseado por `busboy` a `text/plain` → `validateDocument` `no coincide`. Fix cliente normaliza a base `video/webm` antes de `FormData` y servidor tolera `text/plain/octet-stream` cuando sniff es video.

**Causa 3 — UX:** preview `<video>` no decodificaba en algunos celulares y “Grabar de nuevo” confundía. Aprobado Mita: animación `✓ ¡Video grabado con éxito!` sin preview ni re-grabar; re-grabar solo si admin notifica. `DocumentList` cambia `window.open` por `fetch→blob→modal` con `img/iframe` + Descargar, evitando popup-blocker.
